...
const AdminMenu = () => {
  ...
  const fetchData = async () => {
    try {
      const [menuRes, restRes, catRes] = await Promise.all([
        api.get('/menu'), api.get('/restaurants'), api.get('/categories')
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(formData).forEach(key => {
      // In edit mode, IDs might be objects populated from backend
      if (typeof formData[key] === 'object' && formData[key]?._id) {
    ...
    try {
      if (editId) {
        await api.put(`/menu/${editId}`, data, { headers: { 'Content-Type': 'multipart/form-data' } });
        setEditId(null);
      } else {
        await api.post('/menu', data, { headers: { 'Content-Type': 'multipart/form-data' } });
      }
      setFormData({ name: '', description: '', price: '', restaurantId: '', categoryId: '' });
      setImage(null);
      fetchData();
  ...
  return (
    <div className="space-y-8">
      ...
      <form onSubmit={handleSubmit} className="bg-gray-50 p-6 rounded-xl border border-gray-100 space-y-4">
        <h3 className="font-bold text-gray-800">{editId ? 'Edit Food Item' : 'Add New Food Item'}</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <select required name="categoryId" value={formData.categoryId} onChange={handleInputChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white cursor-pointer">
            ...
            <div className="flex items-center w-full">
              <label className="flex flex-col w-full h-32 border-2 border-primary-200 border-dashed hover:bg-primary-50 hover:border-primary-300 transition rounded-xl cursor-pointer">
                <div className="flex flex-col items-center justify-center pt-7">
                  ...
                  <p className="text-xs text-gray-500 font-medium tracking-wide uppercase">{image ? image.name : 'Select an image file (optional for edit)'}</p>
                </div>
        ...
        <div className="flex space-x-3">
          <button disabled={loading} type="submit" className="bg-primary-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-primary-700 md:w-auto shadow-md">
             {editId ? 'Update Menu Item' : 'Add Menu Item'}
          </button>
           {editId && (
              <button type="button" onClick={cancelEdit} className="bg-gray-300 text-gray-800 px-6 py-3 rounded-lg font-bold hover:bg-gray-400 md:w-auto shadow-md">
                 Cancel
              </button>
           )}
        </div>
            ...
            <div className="p-5">
              ...
              <div className="flex gap-2 w-full">
                 <button 
                   ...
                   className="flex-1 flex items-center justify-center text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 p-2 rounded-lg font-bold transition"
                 >
                   <Edit className="w-4 h-4 mr-2" /> Edit
                 </button>
                 <button 
                   onClick={() => handleDelete(item._id)} 
                   className="flex-1 flex items-center justify-center text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 p-2 rounded-lg font-bold transition"
                 >
