...
const AdminRestaurants = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fetchRestaurants();
    } catch (err) { alert('Failed to save restaurant'); }
  ...
  const handleEdit = (rest) => {
    ...
    if (rest.location?.lat && rest.location?.lng) {
      setLocation({ lat: rest.location.lat, lng: rest.location.lng });
    } else {
      setLocation({ lat: 6.9271, lng: 79.8612 });
    }
  };
    ...
    <div className="space-y-10 animate-fade-in-up">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 font-medium">Create, edit and manage your restaurant outlets.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-xl space-y-6">
          <div className="flex items-center space-x-2 text-primary-600 mb-2">
             ...
             <h3 className="font-black text-xl">{editId ? 'Edit Outlet' : 'Add New Outlet'}</h3>
          </div>
          ...
          <div className="flex space-x-3">
            <button disabled={loading} type="submit" className="flex-1 bg-slate-900 text-white p-4 rounded-2xl font-black hover:bg-primary-600 transition-all shadow-lg hover:shadow-primary-500/30 transform hover:-translate-y-1 disabled:opacity-50">
              {loading ? 'Saving...' : (editId ? 'Update Restaurant' : 'Register Restaurant')}
            </button>
            {editId && (
              <button type="button" onClick={cancelEdit} className="px-6 bg-gray-200 text-gray-800 rounded-2xl font-bold hover:bg-gray-300 transition-all">
                Cancel
              </button>
            )}
          </div>
                ...
                {restaurants.map(rest => (
                  <div key={rest._id} className="p-6 flex items-center justify-between group hover:bg-slate-50 transition-colors">
                     ...
                     <div className="flex space-x-2">
                       <button onClick={() => handleEdit(rest)} className="p-3 text-blue-300 group-hover:text-blue-500 hover:bg-blue-50 rounded-xl transition-all">
                          <Edit className="w-5 h-5" />
                       </button>
                       <button onClick={() => handleDelete(rest._id)} className="p-3 text-red-300 group-hover:text-red-500 hover:bg-red-50 rounded-xl transition-all">
