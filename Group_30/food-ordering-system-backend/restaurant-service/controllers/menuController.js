...
exports.addMenu = async (req, res, next) => {
    try {
        ...
        if (!restaurantId || !categoryId || !name || price === undefined) {
             return res.status(400).json({ error: 'RestaurantId, categoryId, name and price are required' });
        }
        ...
        const menu = new Menu({ restaurantId, categoryId, name, description, price, image });
        await menu.save();
...
exports.getMenuByRestaurant = async (req, res, next) => {
    try {
        ...
        const menu = await Menu.find({ restaurantId }).populate('categoryId', 'name');
        res.json(menu);
    } catch (err) {
        next(err);
    }
...
exports.getAllMenu = async (req, res, next) => {
    try {
        const menu = await Menu.find().populate('restaurantId', 'name').populate('categoryId', 'name');
        res.json(menu);
    } catch (err) { next(err); }
};
