...
const Menu = () => {
  ...
  const groupedMenu = menu.reduce((acc, item) => {
    const catName = item.categoryId?.name || 'Other';
      ...
      {Object.keys(groupedMenu).length > 0 ? (
        <div className="space-y-20">
          {Object.entries(groupedMenu).map(([category, items], catIndex) => (
            <div key={category} className={`animate-fade-in-up stagger-${catIndex + 1}`}>
              <div className="flex items-center space-x-4 mb-10">
                <h2 className="text-3xl font-black text-slate-900 tracking-tight">{category}</h2>
                <div className="h-px flex-1 bg-slate-100"></div>
                <span className="text-slate-400 font-bold text-sm uppercase tracking-widest">{items.length} items</span>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {items.map((item, index) => (
                  <div key={item._id} className="bg-white rounded-[2rem] p-5 flex flex-col shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] border border-slate-50 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 group">
