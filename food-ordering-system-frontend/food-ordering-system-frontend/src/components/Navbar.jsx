...
import { ShoppingCart, User, LogOut, Utensils, LayoutDashboard, ChevronDown } from 'lucide-react';
          ...
          <div className="flex items-center space-x-8">
            {(!user || user.role !== 'admin') && (
              <Link to="/cart" className="relative group flex items-center">
                ...
                {cart.items.length > 0 && (
                  <span className="absolute 1 top-0 right-0 bg-gradient-to-r from-primary-500 to-primary-600 text-white text-[10px] rounded-full h-5 w-5 flex items-center justify-center font-bold shadow-md shadow-primary-500/40 border-2 border-white transform scale-100 animate-fade-in-up">
                    {cart.items.length}
                  </span>
                )}
              </Link>
            ...
            {user ? (
              <div className="flex items-center space-x-6">
                {user.role === 'admin' && (
                  <Link to="/admin/categories" className="flex items-center text-sm font-bold text-slate-700 hover:text-primary-600 bg-white border border-slate-200 shadow-sm px-4 py-2 rounded-full hover:shadow-md transition-all">
                ...
                {user.role !== 'admin' && (
                  <Link to="/orders" className={`text-sm font-semibold transition-colors ${isActive('/orders') ? 'text-primary-600' : 'text-slate-600 hover:text-primary-600'}`}>
                    My Orders
                  </Link>
                )}
                ...
                <div className="group relative cursor-pointer flex items-center">
                  ...
                  <ChevronDown className="w-4 h-4 ml-1 text-slate-400 group-hover:text-slate-600 transition-colors" />
                  
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-2xl shadow-xl border border-slate-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right scale-95 group-hover:scale-100">
